...
import { istWeekYmdsFromMonday, istYmdFromDate, istWeekdayShortFromYmd } from '../lib/weeklyStreakIst';
  ...
  return (
    <div className="relative min-h-[100dvh] flex flex-col items-center px-3 pt-3 pb-[max(6rem,env(safe-area-inset-bottom))] overflow-y-auto overflow-x-hidden">
      ...
      <div className="relative z-10 w-full max-w-md flex flex-col flex-1 min-w-0">
        ...
        <p className="text-amber-200/60 text-[10px] mb-2 text-center">{t('weeklyStreak.istNote')}</p>
                ...
                <div className="flex items-start justify-between gap-2 min-w-0">
                  <div className="min-w-0 flex-1">
                    <p className="text-amber-400 text-xs font-semibold">
                      {wd.label} · {istWeekdayShortFromYmd(ymd)}{' '}
        ...
        {proActive ? (
          <div className="w-full mb-6 space-y-4 rounded-xl border border-amber-500/25 bg-black/25 p-3">
          ...
          <p className="text-amber-200/55 text-[10px] text-center mb-6 px-2">{t('weeklyStreak.freePlanNote')}</p>
        )}
      </div>
      <AppFooter />
      <BottomNav />
