    ...
    if (this.state.error) {
      return (
        <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-black text-white font-mono text-sm">
          <h1 className="text-[var(--primary)] text-lg font-semibold mb-4">Something went wrong</h1>
          <pre className="text-red-400 text-xs max-w-full overflow-auto p-4 bg-[#151515] rounded-xl">
            {this.state.error.message}
          </pre>
          <button
            onClick={() => window.location.reload()}
            className="mt-6 px-6 py-3 rounded-xl bg-[var(--primary)] text-black font-medium"
          >
            Reload
          </button>
        </div>
