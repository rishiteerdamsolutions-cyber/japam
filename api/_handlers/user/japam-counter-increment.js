 ...
 * Body: { mode: 'manual' | 'auto', deity: string, delta?: number }
 * Credits japas for this deity to the current IST calendar month.
 */
export async function POST(request) {
  ...
  try {
    ...
    const deity = parseJapamCounterDeityParam(body?.deity);
    if (!deity) return jsonResponse({ error: 'deity is required' }, 400);
    const delta = Math.max(1, Math.min(500, Math.round(Number(body?.delta) || 1)));

    const monthKey = istMonthKey();
    const pubField =
      mode === 'auto' ? publicFieldAutoDeity(monthKey, deity) : publicFieldManualDeity(monthKey, deity);
    ...
    await db.doc(`users/${uid}/data/japa`).set(
      {
        [`japamCounterByDeity.${mode}.${deity}.${monthKey}`]: admin.firestore.FieldValue.increment(delta),
      },
      { merge: true },
    );
    ...
    const manualMonth = Math.max(0, Math.round(Number(pub[fManual]) || 0));
    const autoMonth = Math.max(0, Math.round(Number(pub[fAuto]) || 0));
